...
func TestGetNumWinners(t *testing.T) {
  tests := []struct {
    input string
    want int
  }{
  ...
  for _, tt := range tests {
    t.Run(tt.input, func(t *testing.T) {
      if got := NewCard(tt.input); got.GetNumWinners() != tt.want {
        t.Errorf("TestGetNumWinners(%s) = %d, want %d", tt.input, got, tt.want)
      }
    })
  }
}
...
func TestGetSumOAllProcessedCards(t *testing.T) {
  tests := []struct {
    file string
    want int
  }{
  ...
  for _, tt := range tests {
    t.Run(tt.file, func(t *testing.T) {
      if got := GetSumOAllProcessedCards(tt.file); got != tt.want {
        t.Errorf("GetSumOAllProcessedCards(%s) = %d, want %d", tt.file, got, tt.want)
      }
    })
  }
}